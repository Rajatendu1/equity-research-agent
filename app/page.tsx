...
function ResearchShelf({ onResearch }: { onResearch: (company: string) => void }) {
  const [open,setOpen]=useState(false),[items,setItems]=useState<SavedResearch[]>([]),[message,setMessage]=useState(""),[filter,setFilter]=useState<"All"|"Watchlist"|"Owned"|"Avoided">("All"),[quotes,setQuotes]=useState<Record<number,{price:number;asOf:string}>>({}),[freshness,setFreshness]=useState<Record<number,{verdict:string;trend:string;summary:string;checkedAt:string}>>({}),[diffs,setDiffs]=useState<Record<number,{capturedAt:string;verdict:string;summary:string;trend:string}[]>>({}),[diffOpen,setDiffOpen]=useState<Record<number,boolean>>({}),[refreshing,setRefreshing]=useState(false),[scanning,setScanning]=useState(false);
  useEffect(() => { const handler = async (event: Event) => { const button = event.target as HTMLElement; if (!button.matches(".watchtower-change > button")) return; const article = button.closest("article"); const response = await fetch("/api/research"); const data = await response.json() as { items?: SavedResearch[] }; const item = (data.items ?? []).find(value => article?.textContent?.includes(value.company)); if (!item) return; const text = item.invalidation.toLowerCase(); const impact = text.includes("margin") || text.includes("debt") || text.includes("cash") ? "Needs review: compare this scan with your invalidation trigger." : text ? "Neutral until the saved invalidation condition is explicitly triggered." : "Add an invalidation condition to classify future evidence."; const panel = document.createElement("div"); panel.className = "evidence-diff thesis-impact"; panel.textContent = `Thesis impact · ${impact}`; article?.appendChild(panel); }; document.addEventListener("dblclick", handler); return () => document.removeEventListener("dblclick", handler); }, []);
  useEffect(() => { const handler = async (event: Event) => { const button = event.target as HTMLElement; if (!button.matches(".watchtower-change > button")) return; const article = button.closest("article"); const response = await fetch("/api/research"); const data = await response.json() as { items?: SavedResearch[] }; const item = (data.items ?? []).find(value => article?.textContent?.includes(value.company)); if (!item) return; const analysisResponse = await fetch("/api/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ company: item.symbol || item.company }) }); const analysis = await analysisResponse.json() as { financials?: { metrics?: Record<string, number|null> }; market?: { events?: Array<{title?:string;date?:string;category?:string}> } }; if (!analysisResponse.ok) return; const evidence = { metrics: analysis.financials?.metrics ?? {}, events: (analysis.market?.events ?? []).slice(0, 4) }; await fetch("/api/snapshots", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ researchItemId: item.id, evidenceJson: JSON.stringify(evidence), snapshotId: "source-scan", verdict: "", trend: "", summary: "" }) }); const panel = document.createElement("div"); panel.className = "evidence-diff source-drivers"; const metrics = evidence.metrics; panel.textContent = `Source drivers · Revenue growth ${metrics.revenueGrowth ?? "—"}% · Profit growth ${metrics.profitGrowth ?? "—"}% · Operating margin ${metrics.operatingMargin ?? "—"}% · Cash conversion ${metrics.cashConversion ?? "—"}× · Debt / operating profit ${metrics.debtToOperatingProfit ?? "—"}× · ${evidence.events.length} material events`; article?.appendChild(panel); }; document.addEventListener("click", handler); return () => document.removeEventListener("click", handler); }, []);
  useEffect(() => { const handler = async (event: Event) => { const button = event.target as HTMLElement; if (!button.matches(".watchtower-change > button")) return; const article = button.closest("article"); const response = await fetch("/api/research"); const data = await response.json() as { items?: SavedResearch[] }; const item = (data.items ?? []).find(value => article?.textContent?.includes(value.company)); if (!item) return; const historyResponse = await fetch(`/api/snapshots?itemId=${item.id}`); const historyData = await historyResponse.json() as { snapshots?: Array<{capturedAt:string;verdict:string;trend:string;summary:string}> }; const history = historyData.snapshots ?? []; const panel = document.createElement("div"); panel.className = "evidence-diff"; panel.textContent = history.length < 2 ? "Run a fresh change scan twice to create a dated comparison." : `Previous (${new Date(history[1].capturedAt).toLocaleDateString("en-IN")}) → Current (${new Date(history[0].capturedAt).toLocaleDateString("en-IN")}) · ${history[1].verdict} → ${history[0].verdict} · ${history[1].trend} → ${history[0].trend}`; article?.appendChild(panel); button.textContent = "Hide evidence diff"; }; document.addEventListener("click", handler); return () => document.removeEventListener("click", handler); }, []);
  useEffect(() => { const handler = async (event: Event) => { const button = event.target as HTMLElement; if (!button.matches(".watchtower-change > button")) return; const article = button.closest("article"); const id = Number(article?.getAttribute("data-research-id")); if (!id) return; const existing = article?.querySelector(".evidence-diff"); if (existing) { existing.remove(); button.textContent = "See evidence diff"; return; } button.textContent = "Loading diff…"; try { const response = await fetch(`/api/snapshots?itemId=${id}`); const data = await response.json() as { snapshots?: Array<{ capturedAt:string; verdict:string; trend:string; summary:string }> }; const history = data.snapshots ?? []; const panel = document.createElement("div"); panel.className = "evidence-diff"; panel.innerHTML = history.length < 2 ? "<p>Run a fresh change scan twice to create a dated comparison.</p>" : `<div class="diff-heading"><b>Previous → current</b><small>${new Date(history[1].capturedAt).toLocaleDateString("en-IN")} → ${new Date(history[0].capturedAt).toLocaleDateString("en-IN")}</small></div><div class="diff-grid"><div><small>Research view</small><p>${history[1].verdict} <span>→</span> <b>${history[0].verdict}</b></p></div><div><small>Price trend</small><p>${history[1].trend} <span>→</span> <b>${history[0].trend}</b></p></div><div class="diff-summary"><small>Latest summary</small><p>${history[0].summary}</p></div></div>`; article?.appendChild(panel); button.textContent = "Hide evidence diff"; } catch { button.textContent = "Could not load diff"; } }; document.addEventListener("click", handler); return () => document.removeEventListener("click", handler); }, []);
  ...
  const loadDiff=async(id:number)=>{const isOpen=diffOpen[id];setDiffOpen(current=>({...current,[id]:!isOpen}));if(isOpen||diffs[id])return;try{const response=await fetch(`/api/snapshots?itemId=${id}`),data=await jsonFromResponse(response);if(response.ok)setDiffs(current=>({...current,[id]:(data.snapshots as {capturedAt:string;verdict:string;summary:string;trend:string}[])??[]}));}catch{setMessage("Could not load the evidence history.")}};
  ...
  const scanResearch=async()=>{if(!items.length)return;setScanning(true);setMessage("Comparing fresh research with your saved watchlist…");const results=await Promise.all(items.map(async item=>{try{const response=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({company:item.symbol||item.company})});const data=await jsonFromResponse(response);return response.ok&&data.report&&data.market?[item.id,{verdict:String((data.report as {verdict?:string}).verdict??"Insufficient evidence"),trend:String((data.market as {indicators?:{trend?:string}}).indicators?.trend??"Mixed trend"),summary:String((data.report as {summary?:string}).summary??"Fresh research available for review."),checkedAt:new Date().toISOString()}] as const:null}catch{return null}}));const next:Record<number,{verdict:string;trend:string;summary:string;checkedAt:string}>={};results.forEach(result=>{if(result)next[result[0]]=result[1]});setFreshness(next);setMessage(Object.keys(next).length?`Fresh scan complete for ${Object.keys(next).length} ${Object.keys(next).length===1?"company":"companies"}.`:"The fresh scan could not complete. Try again later.");setScanning(false)};
  ...
  const watchlist=items.filter(item=>["Watching","Researching"].includes(item.status)), owned=items.filter(item=>item.status==="Own"), visible=filter==="All"?items:filter==="Watchlist"?watchlist:filter==="Owned"?owned:items.filter(item=>item.status==="Avoid"), moves=items.map(item=>{const quote=quotes[item.id];return quote&&item.referencePrice?((quote.price/item.referencePrice)-1)*100:null}).filter((move):move is number=>move!==null), averageMove=moves.length?moves.reduce((sum,move)=>sum+move,0)/moves.length:null;
...
export default function Home() {
  ...
  const roadmap = [
    ...
    { phase: "In progress", title: "Portfolio command centre", copy: "Owned ideas and return summaries are in place. Exposure, concentration and portfolio-level risk are the next layer.", status: "Building", highlights: ["Owned ideas live", "Returns live", "Exposure next"] },
